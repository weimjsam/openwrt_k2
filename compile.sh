...
make info
make image PROFILE=${image_name} PACKAGES="tc kmod-ifb kmod-sched kmod-sched-core kmod-netem luci-base luci-proto-ppp luci-mod-admin-full luci-theme-bootstrap luci-i18n-base-zh-cn luci-i18n-firewall-zh-cn wpad-openssl vsftpd openssh-sftp-server uhttpd uhttpd-mod-ubus vsftpd openssh-sftp-server uhttpd uhttpd-mod-ubus  -logd " CONFIG_KERNEL_CRASHLOG=n CONFIG_KERNEL_DEBUG_INFO=n CONFIG_KERNEL_DEBUG_KERNEL=n CONFIG_KERNEL_PRINTK_TIME=n CONFIG_COLLECT_KERNEL_DEBUG=n CONFIG_REPRODUCIBLE_DEBUG_INFO=n
